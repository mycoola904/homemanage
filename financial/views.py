...
def bill_pay_sync_google(request):
    ...
    for account in accounts:
        ...
        # 1) If paid, ensure no calendar event remains (delete if exists)
        if mbp.paid:
            if mbp.google_event_id:
                try:
                    client.delete_event(calendar_id, mbp.google_event_id)                    
                except Exception:
                    pass  # ignore deletion errors, we'll just end up with an orphaned event

                mbp.google_event_id = None
