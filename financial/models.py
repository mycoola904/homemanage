...
class AccountQuerySet(models.QuerySet):
	...
	def for_household(self, household) -> "AccountQuerySet":
		if household is None:
			return self.none()
		return self.ordered().filter(household=household)
...
class Account(models.Model):
	...
	def save(self, *args, **kwargs):
		if self.household_id is None and self.user_id is not None:
			household, _ = Household.objects.get_or_create(
				name=f"{self.user.username} Household",
				defaults={
					"slug": f"{self.user.username}-{str(self.user_id).replace('-', '')[:8]}",
					"created_by": self.user,
				},
			)
			membership = HouseholdMember.objects.filter(household=household, user=self.user).first()
			if membership is None:
				has_primary = HouseholdMember.objects.filter(user=self.user, is_primary=True).exists()
...
class TransactionQuerySet(models.QuerySet):
	...
	def for_household(self, household) -> "TransactionQuerySet":
		if household is None:
			return self.none()
...
class Transaction(models.Model):
	...
	def clean(self):
		...
		if self.account is not None:
			self.household = self.account.household
		if self.account is not None and self.household_id != self.account.household_id:
			raise ValidationError({"account": "Account household does not match transaction household."})
