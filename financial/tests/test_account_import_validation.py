...
class AccountImportValidationTests(TestCase):
    ...
    def test_rejects_unsupported_file_type(self):
        ...
        self.assertContains(response, "Upload a CSV file", status_code=200)
    ...
    def test_rejects_missing_required_headers(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Missing required headers", status_code=200)
    ...
    def test_rejects_invalid_row_values_enum_date_url_and_day(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "status must use canonical values", status_code=200)
    ...
    def test_rejects_duplicate_names_within_upload_case_insensitive(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "duplicate account name appears more than once", status_code=200)
    ...
    def test_rejects_duplicate_name_against_existing_household_case_insensitive(self):
        ...
        response = self._post_csv("accounts.csv", (self._headers() + row).encode("utf-8"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "duplicate account name already exists", status_code=200)

    def test_allows_same_name_in_different_household_for_same_user(self):
        alt_household = Household.objects.create(
            name="Alex Alt Household",
            slug="alex-alt-household",
            created_by=self.user,
        )
        HouseholdMember.objects.create(
            household=alt_household,
            user=self.user,
            role=HouseholdMember.Role.OWNER,
            is_primary=False,
        )
        ...
        session[CURRENT_HOUSEHOLD_SESSION_KEY] = str(alt_household.id)
        session.save()

        row = (
            "shared name,Metro,checking,123456,021000021,,active,250.00,,,,,https://example.com,Import\n"
        )
        response = self._post_csv("accounts.csv", (self._headers() + row).encode("utf-8"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Imported 1 of 1 rows.", status_code=200)
        self.assertTrue(
            Account.objects.filter(
                user=self.user,
                household=alt_household,
                name__iexact="Shared Name",
            ).exists()
        )
    ...
    def test_rejects_files_over_size_limit(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "5 MB or smaller", status_code=200)
    ...
    def test_rejects_files_over_row_limit(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "maximum of 1000 data rows", status_code=200)
