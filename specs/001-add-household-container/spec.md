...
- User has no household memberships at login; system denies household module entry and shows a clear access/empty-state response without exposing internal details.
...
- Archived household is selected as current household; read/write behavior follows explicit business rules (for MVP default: archived households are not selectable from switcher).
...
- **HouseholdMember**: Membership link between user and household with role (`owner`/`admin`/`member`) and `is_primary` marker; each household must always retain at least one owner.
