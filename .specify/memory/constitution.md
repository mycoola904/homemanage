...
Non-trivial defects discovered during feature development or manual testing must either (a) fall strictly within the existing spec’s documented acceptance criteria, or (b) trigger an explicit spec update before implementation continues. Silent scope drift—fixes that alter externally observable behavior without updating the governing spec—is prohibited.

Every change traces back to an approved `/speckit.spec`, `/speckit.plan`, and `/speckit.tasks` artifact before code is written. Pull requests that skip a referenced spec, plan, or task list are rejected. If the spec drifts, halt implementation and revise the document first. Tests must be written to fail against the spec scenario prior to implementing the fix or feature. Clarification: Implementation can iterate as long as it stays within the documented acceptance criteria; spec drift—any change to requirements, scope, or externally observable behavior—requires updating the spec before coding resumes.

### **II. Deterministic Correctness**

All edit/update views must bind Django ModelForms with `instance=` on both GET and POST requests. Failure to bind the instance is considered a correctness violation because it can alter externally observable behavior (e.g., uniqueness collisions) and break deterministic update semantics.
...
All UX state originates on the server, while non-authoritative client-side hints (e.g., loading indicators or focus nudges) may provide temporary guidance.

For every HTMX **form** endpoint, the implementation plan must explicitly document:

- the `hx-target` element ID(s)
...
- Database: SQLite is permitted and preferred for automated tests and local agent execution to guarantee database creation permissions and deterministic test runs. Production database engine (e.g., PostgreSQL) must support transactional integrity and deterministic ordering.
- Test Environment Rule: AI agents must default to SQLite for automated test execution unless explicitly instructed otherwise in the spec. A dedicated `core.settings_test` (or equivalent) module must exist and explicitly configure SQLite for tests to enforce this rule in code. Production-only database constraints must be validated separately via documented migration checks.
...
Feature branches must not be merged until manual CRUD and edge-path testing is completed and documented. `main` must remain a trusted, stable baseline at all times.
...
- Architectural memory: completed feature specs live under `specs/` and are treated as **immutable historical artifacts**. Do not rewrite or “silently correct” completed specs after a feature is merged; instead, create a new spec (next numbered folder) or add an explicit addendum documenting the change and its rationale. Prior specs may be copied forward as a starting template for new features.
...
**Version**: 1.1.1 | **Ratified**: 2026-02-10 | **Last Amended**: 2026-02-10
